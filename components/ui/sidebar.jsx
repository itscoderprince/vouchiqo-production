...
const SIDEBAR_WIDTH_MOBILE = "290px";
...
function SidebarTrigger({ className, onClick, ...props }) {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon-sm"
      className={cn("p-1.5 text-slate-700 hover:text-slate-900 hover:bg-slate-100/80 rounded-lg transition-colors cursor-pointer", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }) {
  // Deterministic width to prevent hydration mismatch.
  const [width] = React.useState("75%");
