...
export async function dispatchEvent(options = {}) {
  const { target = "user", userId, room, event, payload = {}, notify } = options;
  ...
  try {
    ...
    logger.warn({ err: socketErr, event }, "[dispatchEvent] Socket emit failed silently");
  }
  ...
  if (notify && (notify.userId || userId)) {
    try {
      ...
      emitToUser(recipientId, SOCKET_EVENTS.NOTIFICATION_NEW, {
        ...
        unreadCount: await Notification.countDocuments({ userId: recipientId, isRead: false }),
      });
    } catch (dbErr) {
      logger.error({ err: dbErr, notify }, "[dispatchEvent] Failed to persist DB notification");
    }
  }
}
