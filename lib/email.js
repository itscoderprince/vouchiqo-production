...
export async function sendEmail({ to, from = NOREPLY_EMAIL, subject, html }) {
  ...
  let recipient = to;
  const isTestingDomain = from.includes("onboarding@resend.dev");
  ...
  try {
    ...
    if (errMsg.includes("testing emails to your own email address")) {
      console.error(
        `[Resend Validation Error]: Cannot send email to ${to}.\n-> Reason: ${errMsg}\n-> Fix: Verify your domain at https://resend.com/domains and update EMAIL_FROM/EMAIL_FROM_MERCHANT in .env.local to use your verified domain.`
      );
    } else {
