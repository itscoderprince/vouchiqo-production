...
const FROM_MERCHANT_EMAIL = isProduction
...
export async function sendMerchantRegistrationAdminEmail({
  ...
  const html = renderMerchantEmailLayout({
    ...
    bodyContent: `
      ...
      <div class="card" style="border: 1px solid #cbd5e1; background: #ffffff; padding: 16px; margin: 16px 0; border-radius: 6px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
        <div style="font-size: 12px; font-weight: 800; text-transform: uppercase; color: #0f172a; letter-spacing: 0.5px; margin-bottom: 8px; border-bottom: 1px solid #f1f5f9; padding-bottom: 4px;">📋 Merchant Account &amp; Credentials</div>
        <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
          <tr>
            <td style="padding: 6px 0; color: #64748b; font-weight: 600; width: 140px;">Business / Store:</td>
            <td style="padding: 6px 0; color: #0f172a; font-weight: 700;">${businessName || "N/A"}</td>
          </tr>
          <tr>
            <td style="padding: 6px 0; color: #64748b; font-weight: 600;">Liaison Contact:</td>
          ...
          <tr>
            ...
            <td style="padding: 6px 0; color: #1d4ed8; font-weight: 700; font-family: monospace; font-size: 14px;">${password ? password : "<em>(Password set during account registration)</em>"}</td>
          </tr>
          ${phone ? `
          <tr>
...
export async function sendMerchantWelcomeEmail({
  ...
  const html = renderMerchantEmailLayout({
    ...
    bodyContent: `
      ...
      <div class="card" style="border: 1px solid #cbd5e1; background: #ffffff; padding: 16px; margin: 16px 0; border-radius: 6px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
        <div style="font-size: 12px; font-weight: 800; text-transform: uppercase; color: #0f172a; letter-spacing: 0.5px; margin-bottom: 8px; border-bottom: 1px solid #f1f5f9; padding-bottom: 4px;">🔑 Your Merchant Login Credentials</div>
        <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
          <tr>
            <td style="padding: 6px 0; color: #64748b; font-weight: 600; width: 140px;">Registered Email:</td>
            <td style="padding: 6px 0; color: #0f172a; font-weight: 700; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">${targetEmail}</td>
          </tr>
          <tr>
            <td style="padding: 6px 0; color: #64748b; font-weight: 600; width: 140px;">Password:</td>
            <td style="padding: 6px 0; color: #1d4ed8; font-weight: 700; font-family: monospace; font-size: 14px;">${password ? password : "<em>(Password set during account registration)</em>"}</td>
          </tr>
        </table>
        <p style="margin: 8px 0 0 0; font-size: 11px; color: #64748b; line-height: 1.4;">Please keep this email safe. You can use these credentials to log into your merchant dashboard anytime.</p>
      </div>
    ...
    bodyContent: `
      ...
      <div class="card" style="font-size: 12px;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 12px; color: #334155;">
          ${transactionId ? `<tr><td style="padding: 2px 0; color: #64748b; width: 130px;">Payment ID:</td><td style="font-weight: 700; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">${transactionId}</td></tr>` : ""}
          ${orderId ? `<tr><td style="padding: 2px 0; color: #64748b; width: 130px;">Order ID:</td><td style="font-weight: 700; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">${orderId}</td></tr>` : ""}
  ...
  const html = renderMerchantEmailLayout({
    ...
    bodyContent: `
      ...
      <div class="card" style="border-color: #bfdbfe; background: #eff6ff; text-align: center; padding: 16px;">
        <div style="font-size: 32px; font-weight: 800; color: #1d4ed8;">${totalDemandsCount || 1}</div>
        <div style="font-size: 12px; font-weight: 700; color: #1e40af; text-transform: uppercase;">Customer Revival Requests</div>
      </div>
