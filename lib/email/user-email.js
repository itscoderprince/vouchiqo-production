...
async function dispatchEmail({ to, subject, html }) {
  ...
  try {
    ...
    // If Resend returns sandbox validation error (due to onboarding@resend.dev unverified domain), fallback to devRecipient
    if (errMsg.includes("testing emails to your own email address") || errMsg.includes("validation_error")) {
      console.warn(
        ...
        `-> Fix for Production: Verify custom domain at https://resend.com/domains and set EMAIL_FROM in .env.local`
      );
